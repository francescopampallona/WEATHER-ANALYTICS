...
import { ArrowLeft, Sun, RefreshCw, Table, LineChart as ChartIcon } from 'lucide-react';
...
interface AnnualAnalysisScreenProps {
  onBack: () => void;
}

export const AnnualAnalysisScreen: React.FC<AnnualAnalysisScreenProps> = ({ onBack }) => {
  ...
  const fetchData = async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch annual historical weather data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns: ColumnDef<AnnualSummaryData>[] = [
    ...
    <div className="flex-1 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Annual Analysis</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Yearly summaries, hot days count & frost days for {location.name}</p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-800/90 rounded-2xl p-3.5 border border-slate-200/80 dark:border-slate-700/80 space-y-2 shadow-xs">
        <div className="flex items-center space-x-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
          ...
          <span>Select Annual Year Interval</span>
        </div>
      ...
      {loading ? (
        <LoadingState message={`Calculating annual weather records (${startYear}–${endYear})...`} />
      ) : error ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
              Annual Climate Averages ({startYear} → {endYear})
            </span>
